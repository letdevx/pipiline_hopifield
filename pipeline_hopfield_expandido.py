...
selecionador._data = pl.read_csv(path_f_txt)
selecionador.filtrar_matriz(path_f_top5k)

print(f'\n[Mathys] Filtrando genes nas novas dimensões... (genes faltando aparecerão como 0.5 perfeitamente)')
selecionador._data = pl.read_csv(path_m_txt)
selecionador.filtrar_matriz(path_m_top5k)

print('\nVerificando cobertura no Mathys nas Dimensões Expandidas:')
cobertura = AnalisadorCobertura()
cobertura.analisar(path_m_top5k, out_csv=os.path.join(OUT_TREINAMENTO, 'cobertura_mathys_expandida.csv'))
...
# 
# Armazena os 210 padrões na Modern Hopfield Network.
# 
...
# A rede agora mapeará e armazenará apenas a versão original W0-Binária (~11k genes)
rede35.store(perf35)
