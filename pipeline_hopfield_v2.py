...
cobertura.analisar(
    out_csv=os.path.join(OUT_ALINHAMENTO, "top5000_cobertura_mathys.csv")
)
...
print(
    f"[Mathys] labels shape: {labels_mathys.shape}, tipos: {np.unique(labels_mathys)}"
)
...
for cls in sorted(CORES):
    ...
    if mask.any():
        ax.scatter(
            projetor.Wpc[idx_pca][mask, 0],
            projetor.Wpc[idx_pca][mask, 1],
            c=CORES[cls],
            label=f"{cls} — {CLASSE_NOMES[cls]}",
            s=6,
            alpha=0.6,
            linewidths=0,
        )
...
print(
    f"Classes: {_meta_json['classes']}  nc={_meta_json['nc']}  padroes={_meta_json['n_patterns']}"
)
...
avaliador_f.avaliar(Wrecuperado_f, clo).plotar(
    titulo="Confusão — rede35 (Fujita → Fujita)"
)
print(avaliador_f)
...
avaliador_m.avaliar(Wrecuperado_m, clo_m).plotar(
    titulo="Confusão — rede35 (Mathys → Fujita, 0.5)"
)
...
avaliador_m_bin.avaliar(Wrecuperado_m_bin, clo_m).plotar(
    titulo="Confusão — rede35 (Mathys binário puro)"
)
...
diff_frac = float(
    (W_mathys[mask_nao_sentinela] != Wrecuperado_m[mask_nao_sentinela]).mean()
)
...
avaliador_f.plotar(
    titulo="Fujita → Fujita (normalizada)", ax=axes[0, 1], normalizado=True
)
avaliador_m.plotar(titulo="Mathys → Fujita 0.5 (contagens)", ax=axes[1, 0])
avaliador_m.plotar(
    titulo="Mathys → Fujita 0.5 (normalizada)", ax=axes[1, 1], normalizado=True
)
avaliador_m_bin.plotar(titulo="Mathys → Fujita binário (contagens)", ax=axes[2, 0])
avaliador_m_bin.plotar(
    titulo="Mathys → Fujita binário (normalizada)", ax=axes[2, 1], normalizado=True
)
...
ax.set_ylabel(f"Distância ao {k_nn}º vizinho")
...
print(
    f"Clusters encontrados: {n_clusters}  |  Pontos de ruído: {n_ruido} ({n_ruido / len(labels_db):.1%})"
)
...
ax.scatter(
    rate_ref, rate_mbin, alpha=0.7, label="Mathys bin", color="tomato", s=40, marker="s"
)
...
relatorio.adicionar_figura(
    "PCA — Espaço SWeeP Fujita", fig_pca, secao="Visualizações Exploratórias"
)
relatorio.adicionar_figura("t-SNE — Fujita (SWeeP)", fig_tsne_a, secao="t-SNE e DBSCAN")
relatorio.adicionar_figura(
    "t-SNE — Fujita (●) + Mathys (▲)", fig_tsne_b, secao="t-SNE e DBSCAN"
)
relatorio.adicionar_figura(
    "DBSCAN — Curva k-NN", fig_dbscan_knn, secao="t-SNE e DBSCAN"
)
relatorio.adicionar_figura(
    "DBSCAN — Clusters vs Rótulos Biológicos", fig_dbscan_comp, secao="t-SNE e DBSCAN"
)
relatorio.gerar()
print(relatorio)
