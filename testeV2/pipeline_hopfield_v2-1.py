...
_curr_dir = os.path.dirname(os.path.abspath(__file__)) if '__file__' in globals() and os.path.exists(__file__) else os.getcwd()
_root_dir = os.path.abspath(os.path.join(_curr_dir, '..')) if os.path.basename(_curr_dir) == 'testeV2' else _curr_dir
...
# 
# Em vez de aplicar recortes por frequência (top-5000), este pipeline experimental
# utiliza a TOTALIDADE dos 36.591 genes nativos do Fujita como referência canônica.
# Os genes presentes no Fujita que estavam ausentes no Mathys preservam integralmente
# o valor sentinela 0.5 gerado pelo Alinhador (conforme ADR 007).
...
print('=== Configurando Córtex Completo (36.591 Genes Nativos do Fujita) ===')
...
# Exportação/carregamento das matrizes densas em .npy para melhor performance I/O
...
# Cobertura no Mathys nas Dimensões Completas (36.591 genes)
cobertura = AnalisadorCobertura(path_top_expandidos, leitor.map_f, leitor.map_m)
cobertura.analisar(out_csv=os.path.join(OUT_TREINAMENTO, 'top36k_cobertura_mathys.csv'))
...
# 
# Projeta a matriz binarizada do Fujita (células × 36.591 genes) no espaço
...
# Utiliza automaticamente a base ortonormal canônica congelada configurada em src.config.PATH_ORTHBASE_RDS
...
# 
# Carrega:
# - `W0`: matriz binária Fujita (células × 36.591 genes) — usada como padrões da rede.
...
# 
# Para cada uma das 7 classes executa KMeans com `nc=30` subclusters no espaço
# SWeeP e seleciona o vetor binário mais próximo de cada centroide como
# representante no espaço completo de 36.591 genes. Resulta em `7 × 30 = 210 padrões`.
...
print(extrator)
print(f'perf35 shape: {perf35.shape}  (esperado: (210, {carregador.W0.shape[1]}))')


# ## 10. Treinamento da rede (rede35)
# 
# Armazena os 210 padrões do genoma completo (36.591 genes) na Modern Hopfield Network.
...
# Salva pesos (.pt) e metadados (.json) da nova rede 36k
...
# 
# **Use esta célula EM VEZ das seções 9 e 10** quando o treinamento foi realizado em execução anterior.
...
# Só carrega do disco se a rede não estiver em memória com as dimensões atuais
if 'rede35' not in globals() or rede35.patterns.shape[1] != carregador.W0.shape[1]:
    if os.path.exists(PATH_PT) and os.path.exists(PATH_META):
        ...
        print("Rede pré-treinada carregada do disco:")
        print(rede35)
else:
    print(f"Rede mantida na memória: {rede35.patterns.shape[0]} padrões × {rede35.patterns.shape[1]} genes.")
...
# Seleciona de forma eficiente uma amostra de até 1.000 células da subclasse 3
idx_clo3 = np.where(clo == 3)[0]
n_test = min(1000, len(idx_clo3))
rng_test = np.random.default_rng(SEED)
idx_sample = rng_test.choice(idx_clo3, size=n_test, replace=False)

Wk4_sample = np.asarray(carregador.W0[idx_sample], dtype=np.float32)
Wtes       = rede35.retrieve(Wk4_sample, batch_size=512)
...
genes_faltantes_qtd = int(np.sum(W_mathys == 0.5))
os.makedirs(OUT_TOP_GENES, exist_ok=True)
PATH_IMPUTADO_H5AD = os.path.join(OUT_TOP_GENES, 'X_mathys_IMPUTADO_completo_36k.h5ad')

# Recuperação em lotes e construção OOM-Safe da matriz esparsa CSR
Wrecuperado_m = np.empty(W_mathys.shape, dtype=np.float32)
...
for i in range(0, W_mathys.shape[0], chunk_size):
    w_chunk = W_mathys[i : i + chunk_size]
    rec_chunk = rede35.retrieve(w_chunk, batch_size=512)
    Wrecuperado_m[i : i + chunk_size] = rec_chunk
    
    chunk_imp = np.where(w_chunk == 0.5, rec_chunk, w_chunk).astype(np.float32, copy=False)
    ...
    del w_chunk, rec_chunk, chunk_imp, csr_chunk
    gc.collect()

genes_resolvidos_qtd = int(np.sum((W_mathys == 0.5) & (Wrecuperado_m == 0.5)))
...
adata_imp = ad.AnnData(
    X=X_sparse,
    obs=pd.DataFrame(index=[f'celula_{i}' for i in range(W_mathys.shape[0])]),
    var=pd.DataFrame(index=genes_unificados)
)
adata_imp.write_h5ad(PATH_IMPUTADO_H5AD, compression='gzip')
print(f'Matriz Mathys Imputada Exportada para (CSR Gzip): {PATH_IMPUTADO_H5AD}')
