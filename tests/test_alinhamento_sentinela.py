...
def test_impacto_matematico_sentinela_em_hopfield():
    ...
    """
    GeradorGroundTruthSintetico(n_celulas=4, n_genes=4, n_classes=2, seed=42)
    ...
    assert escore_meio > escore_zero, (
        "O escore de atenção com sentinela 0.5 deve superar o preenchimento por zero para genes ausentes."
    )
...
def test_alinhador_direto_preenche_ausentes():
    ...
    alinhador = Alinhador(
        path_binarizada_m="dummy",
        path_binarizada_f="dummy",
        out_dir="dummy",
        map_f={},
        map_m={},
    ...
    assert adata_out.shape == (2, 3), (
        "A matriz alinhada deve conter todas as colunas do genoma de referência."
    )

    matriz_final = (
        adata_out.X.toarray() if hasattr(adata_out.X, "toarray") else adata_out.X
    )
...
def test_retrieve_com_mascara_sentinela_ausentes():
    ...
    mask_ausentes = np.array(
        [False, True, False, True]
    )  # G1 e G3 são ausentes na plataforma alvo

    recuperado = rede.retrieve(
        queries, batch_size=1, mask_sentinela_ausentes=mask_ausentes, fill_value=0.5
    )
