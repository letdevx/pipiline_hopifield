...
def test_extracao_padroes_kmeans_recupera_prototipos():
    ...
    assert padroes.shape == (3, 8), f"Esperado 3 protótipos de 8 genes, obtido {padroes.shape}"
    ...
    np.testing.assert_array_equal(padroes[0], assinatura_a, err_msg="O protótipo da Classe 1 (Tipo A) foi distorcido.")
    np.testing.assert_array_equal(padroes[1], assinatura_b, err_msg="O protótipo da Classe 2 (Tipo B) foi distorcido.")
    np.testing.assert_array_equal(padroes[2], assinatura_c, err_msg="O protótipo da Classe 3 (Tipo C) foi distorcido.")
