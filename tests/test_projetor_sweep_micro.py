...
def test_projetor_sweep_dimensao_adaptavel():
    ...
    gerador = GeradorGroundTruthSintetico(n_celulas=n_celulas, n_genes=n_genes, n_classes=3, seed=77)
    ...
    R = projetor.R
    assert R.shape == (n_genes, n_comp_alvo), f"Base R esperada {(n_genes, n_comp_alvo)}, obtida {R.shape}"
    
    erro_ortogonalidade = np.abs(R.T @ R - np.eye(n_comp_alvo)).max()
    assert erro_ortogonalidade < 1e-5, f"Erro de ortogonalidade excede limite aceitável: {erro_ortogonalidade}"
    
    # Projeta os dados
    projetor.projetar(matriz)
    Wswp = projetor.Wswp
    
    assert Wswp.shape == (n_celulas, n_comp_alvo), "A projeção Wswp deve ter dimensões (células × componentes)."
...
def test_projetor_sweep_preserva_separabilidade_de_classes():
    ...
    assert dist_mesma_classe < dist_classes_distintas, "A projeção SWeeP deve preservar a topologia e separabilidade biológica."
