...
def test_binarizacao_preserva_assinaturas(temp_dir):
    ...
    gerador = GeradorGroundTruthSintetico(
        n_celulas=12, n_genes=8, n_classes=3, seed=101
    )

    # Gera matriz com contagens de expressão simuladas (5.0 a 25.0 nos ativos)
    adata_continuo = gerador.gerar_matriz_pura(
        formato="anndata", contagem_continua=True
    )
    ...
    matriz_referencia = gerador.gerar_matriz_pura(
        formato="numpy", contagem_continua=False
    )

    # Asserções de conformidade
    assert adata_resultado.shape == (12, 8), (
        "As dimensões da matriz sofreram mutação indevida."
    )
    ...
    assert adata_resultado.X.dtype == np.int8, (
        "O tipo de dado gerado deve ser int8 para economia de memória."
    )
