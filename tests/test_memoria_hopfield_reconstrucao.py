...
def test_reconstrucao_hopfield_elimina_dropouts():
    ...
    matriz_perturbada = gerador.gerar_matriz_perturbada(dropouts_deterministicos=dropouts, formato="numpy")
    ...
    matriz_reconstruida = rede.retrieve(matriz_perturbada, batch_size=6, normalize=False)
    ...
    assert erro_residual == 0, f"Erro residual de {erro_residual}. A rede não reconstruiu perfeitamente o Ground Truth."
    assert taxa_acertos == 100.0, "Taxa de acertos deve atingir 100% no teste controlado."
...
def test_escalabilidade_e_complexidade_tempo_memoria():
    ...
    gerador = GeradorGroundTruthSintetico(n_celulas=n_celulas_teste, n_genes=n_genes_teste, n_classes=5, seed=123)
    ...
    assert resultado.shape == (n_celulas_teste, n_genes_teste), "A saída deve preservar as dimensões do dataset."
    assert tempo_gasto < 10.0, f"O tempo de execução excedeu 10s ({tempo_gasto:.2f}s), indicando gargalo $O(f(n))$."
