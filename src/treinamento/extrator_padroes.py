...
class ExtratorPadroesSubcluster:
    ...
    def __init__(self, W0, labels, classes=None, nc=10, seed=42):
    ...
    def extrair(self, Wswp):
        ...
        for cls in classes_validas:
            ...
            print(f"[ExtratorPadroesSubcluster] Classe {cls}: n={len(ids_cls)}, "
                  f"KMeans(nc={self.nc})...")
            km = KMeans(n_clusters=self.nc, n_init=5, random_state=self.seed).fit(Wswp_cls)
        ...
        print(f"[ExtratorPadroesSubcluster] Extração concluída: "
              f"{self.padroes.shape[0]} padrões ({len(classes_validas)} classes × {self.nc})")
        return self
