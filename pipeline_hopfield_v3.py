...
# %% [markdown]
# # `pipeline_hopfield_v3` — Experimento com Amostragem por Consenso (k=5) e 210 Padrões
#
# Reimplementação do fluxo de `script01_analises_preliminares.ipynb` usando as
# classes disponíveis em `src/`, testando a amostragem de consenso por vizinhança SWeeP.
#
...
# (rSWeeP, AIBIALab). A memória associativa armazena perfis **binários de consenso** por
# tipo celular; o espaço SWeeP é usado para clusterizar e escolher protótipos via voto de maioria dos vizinhos.
#
# **Diferenças em relação ao `v2` e `script01`:**
...
# - Configuração v3: `nc=30` clusters por classe, `k=5` representantes por centroide (consenso/voto de maioria)
...
# %% [markdown]
# ## 9. Extração de padrões por subcluster com amostragem de consenso (perf35)
#
# Para cada uma das 7 classes executa KMeans com `nc=30` clusters no espaço
# SWeeP e seleciona os `k=5` vizinhos binários mais próximos de cada centroide para
# calcular o perfil transcricional por voto de maioria (`mean >= 0.5`).
# Resulta em `7 × 30 = 210 padrões` altamente robustos a ruídos individuais (dropouts).
...
print(extrator)
print(f'perf35 shape: {perf35.shape}  (esperado: (210, 5000))')


# %% [markdown]
# ## 10. Treinamento da rede (rede35)
#
# Armazena os 210 padrões de consenso na Modern Hopfield Network.
#
# **Regra de armazenamento:** simplesmente guardar os padrões — não há
# treinamento iterativo.
#
# **Parâmetros:**
# - `beta=30.0`: temperatura inversa do softmax (maior → mais winner-takes-all)
# - `n_iters=1`: uma iteração de atualização já é suficiente
# - `threshold=0.0`: genes binários tratados no espaço W0

# %%
rede35 = ModernHopfieldNetwork(beta=30.0, n_iters=1, binary=True, threshold=0.0)
# A rede agora mapeará e armazenará apenas a versão original W0-Binária em consenso de vizinhos (k=5)
rede35.store(perf35)
...
# noutra maquina. Copie os arquivos rede35_v3_k5_210padroes.pt e rede35_v3_k5_210padroes_metadata.json para
# a pasta outputs/hopfield/ desta maquina e execute apenas esta celula.
#
# Se treinou aqui (secoes 9 e 10), **pule esta celula**.

# %%
# Mude para True SOMENTE se você pulou as Seções 9 e 10 e deseja carregar um modelo já treinado do disco!
CARREGAR_MODELO_DO_DISCO = False

if CARREGAR_MODELO_DO_DISCO:
    import json as _json

    PATH_PT   = os.path.join(OUT_HOPFIELD, 'rede35_v3_k5_210padroes.pt')
    PATH_META = os.path.join(OUT_HOPFIELD, 'rede35_v3_k5_210padroes_metadata.json')
    ...
    print("[Seção 10b] CARREGAR_MODELO_DO_DISCO = False -> Mantendo em memória a rede35 (210 padrões x 5000 genes) gerada na Seção 10.")
...
PATH_IMPUTADO = os.path.join(OUT_TOP_GENES, 'X_mathys_IMPUTADO_rede35_v3_k5.npy')
...
avaliador_m.avaliar(Wrecuperado_m, clo_m).plotar(titulo='Confusão — rede35 v3 k=5 (Mathys → Fujita, 0.5)')
...
avaliador_m_bin.avaliar(Wrecuperado_m_bin, clo_m).plotar(titulo='Confusão — rede35 v3 k=5 (Mathys binário puro)')
...
path_rede_v3 = os.path.join(OUT_HOPFIELD, 'rede35_v3_k5_210padroes.pt')
path_meta_v3 = os.path.join(OUT_HOPFIELD, 'rede35_v3_k5_210padroes_metadata.json')

# Salva rede
rede35.salvar(path_rede_v3)
...
with open(path_meta_v3, 'w') as _f:
    _json.dump(_metadata, _f, indent=2)

print(f'Rede salva em    : {path_rede_v3}')
print(f'Metadados salvos : {path_meta_v3}')
